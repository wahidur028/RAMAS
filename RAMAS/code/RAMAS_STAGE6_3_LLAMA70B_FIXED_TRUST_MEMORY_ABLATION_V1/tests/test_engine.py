...
from stage63lib.engine import Arm, payload_for, run_pair, make_state
from stage63lib.fixture import build_fixture
from stage63lib.journal import Journal
from stage63lib.legacy import RegimeTrust, state_vector
from stage63lib.fixed_trust import FixedTrust, validate_fixed_trust_config
...
class EngineTests(unittest.TestCase):
    ...
    def test_no_memory_has_empty_evidence_but_independent_outcome_ledger(self):
        ...
        for name, arm in arms.items():
            ...
            self.assertEqual(arm.trust.events, [])
            self.assertEqual(arm.trust.beta, {'bear': 0.05, 'bull': 0.05, 'mix': 0.05})
        ...
        self.assertIsNot(arms['memory'].ledger, arms['no_memory'].ledger)
        self.assertIsNot(arms['memory'].trust, arms['no_memory'].trust)
    ...
    def test_caller_injected_future_current_day_and_wrong_regime_hidden(self):
        arm = Arm('memory', FixedTrust(self.config))
    ...
    def test_year_boundary_carries_memory_trust_and_drifted_portfolio(self):
        ...
        self.assertEqual(arms['memory'].trust.events, [])
        self.assertEqual(arms['no_memory'].trust.events, [])
        self.assertGreater(rows[first_january]['memory_retrieved_memory_cross_year_count'], 0)
        self.assertEqual(rows[first_january]['no_memory_retrieved_memory_cross_year_count'], 0)
        for row in rows:
            self.assertEqual(row['memory_beta'], 0.05)
            self.assertEqual(row['no_memory_beta'], 0.05)
    ...
    def test_interrupted_resume_reconstructs_exact_rows_and_fixed_trust(self):
    ...
    def test_fixed_positive_trust_survives_losses_months_and_all_regimes(self):
        changed = copy.deepcopy(self.period)
        changed.asset_returns[:] = -0.05
        changed.current_trace['portfolio_net_return'] = 0.
        rows, arms, _, _ = self.run_case(self.root / 'losses', period=changed)
        self.assertEqual({r['hard_regime'] for r in rows}, {'bear', 'bull', 'mix'})
        self.assertTrue(any(e['shadow_log_advantage_vs_ramoe'] < -0.001
                            for e in arms['memory'].ledger.episodes))
        for name in self.config['arms']:
            self.assertTrue(all(r[name + '_beta'] == 0.05 for r in rows))
            self.assertEqual(arms[name].trust.events, [])
            self.assertEqual(arms[name].stale_changes, [])
            snapshot = arms[name].trust.beta
            snapshot['bull'] = 0.
            self.assertEqual(arms[name].trust.value('bull'), 0.05)

    def test_impossible_trust_configuration_stops_before_any_model_call(self):
        changes = [dict(fixed_beta=0.), dict(fixed_beta=0.1), dict(fixed_beta=True),
                   dict(fixed_beta='0.05'), dict(fixed_beta=float('nan')),
                   dict(fixed_beta=float('inf')), dict(agent_trust_mode='ADAPTIVE'),
                   dict(common_legacy_trust_rule=True), dict(continue_across_years=False),
                   dict(arms=['memory', 'memory'])]
        for i, change in enumerate(changes):
            with self.subTest(change=change):
                config = dict(self.config, **change)
                provider = MemorySensitiveProvider()
                with self.assertRaises(ValueError):
                    self.run_case(self.root / f'impossible-{i}', provider=provider, config=config)
                self.assertEqual(provider.requests, [])

    def test_adaptive_trust_object_cannot_enter_fixed_engine(self):
        arm = Arm('memory', RegimeTrust(self.config['trust']))
        with self.assertRaisesRegex(TypeError, 'requires FixedTrust'):
            payload_for(arm, self.period, 0, self.config, self.base, self.risk)

    def test_transmission_metadata_matches_journal_and_wealth_reconstruction(self):
        rows, arms, _, journal = self.run_case(self.root / 'transmission')
        for name in self.config['arms']:
            wealth = 1.
            for i, row in enumerate(rows):
                entry = json.loads((journal.output / 'calls' / f'{i+1:06d}-{name}.json').read_text())
                payload = entry['request']
                previews = payload['safe_exposure_previews']
                xs = []
                for action in ['BTC', 'CASH', 'ABSTAIN']:
                    prefix = name + '_preview_' + action.lower()
                    self.assertEqual(row[prefix + '_exposure'], previews[action]['risk_limited_exposure'])
                    self.assertEqual(row[prefix + '_desired_exposure'], previews[action]['blended_desired_exposure'])
                    xs.append(row[prefix + '_exposure'])
                self.assertEqual(row[name + '_action_headroom'], max(xs) - min(xs) > 1e-12)
                self.assertEqual(row[name + '_exposure'], previews[row[name + '_action']]['risk_limited_exposure'])
                self.assertEqual(row[name + '_core_desired_exposure'], 0.45)
                self.assertEqual(row[name + '_wealth_before'], wealth)
                wealth *= 1 + row[name + '_net_return']
                self.assertEqual(row[name + '_wealth_after'], wealth)
                if row[name + '_retrieved_memory_count'] == 0:
                    self.assertIsNone(row[name + '_retrieved_memory_max_age_days'])
                else:
                    self.assertGreaterEqual(row[name + '_retrieved_memory_max_age_days'], 0)
            self.assertEqual(wealth, arms[name].wealth)
