# Scientific method: RAMAS Stage 5.5.1 Llama-70B adaptive tool-loop repair
...
5. receives one structured correction round if evidence is missing or the
   deterministic shield rejects the draft;
6. confirms, reduces, changes, or cancels the action;
7. states a falsifiable hypothesis and invalidation condition;
8. passes the final proposal to deterministic evidence and risk gates.
...
- another intervention is already active, except for `KEEP` or a valid
  `ROLLBACK`;
...
At least 11 of 12 synthetic triggered cases must produce valid plans, valid
final decisions, complete operator evidence, and shield-safe final actions.
At least one of three severe positive-path cases must execute a bounded
intervention. A syntactically valid but unusable action therefore cannot pass.
These cases test the interface only and permit no economic claim.
