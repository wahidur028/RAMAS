#!/usr/bin/env python3
"""Stage 6.2 matched episodic-memory ablation, with durable restart."""
import argparse
...
import numpy as np
import pandas as pd
from stage62lib.legacy import legacy,source,SYSTEM_PROMPT,ControlledProvider,validate_decision
from stage62lib.journal import Journal,atomic_json,digest
from stage62lib.engine import run_pair
from stage62lib.reporting import write_reports
...
def load_config():
    ...
    if config['agent']!=old['agent'] or config['trust']!=old['trust']:
        raise ValueError('Matched test must retain legacy agent and trust contracts in both arms')
...
def preflight(journal,config):
    ...
    for i,case in enumerate(legacy.semantic_cases()):
        ...
        print(f'STAGE62_PREFLIGHT={i+1}/12 VALID={valid} EXPECTED={case["expected_action"]} ACTION={action}',flush=True)
...
def execute(args):
    ...
    for relative in [config['source_result_relative'],config['corrected_run_relative']]:
    ...
    try:
        ...
        print('RAMAS_STAGE6_2_INPUT_CHECK=START',flush=True)
        if args.demo:
            ...
            from stage62lib.inputs import load_inputs
            ...
            from stage62lib.provider import OllamaProvider
        ...
        print('RAMAS_STAGE6_2_INPUT_CHECK=PASS',flush=True)
        ...
        if existing:
            ...
            atomic_json(output/'00_CONTRACT.json',dict(identity=identity,config=config,
                ...
                hypothesis='Total effect of retrieved episodic evidence under the same adaptive-trust rule',
        ...
        if (output/'RUN_COMPLETE.json').exists():
            ...
            print('RAMAS_STAGE6_2_STATUS='+complete['status'],flush=True)
        ...
        summary=write_reports(output,rows,config)
        ...
        final=dict(status='PASS' if technical else 'TECHNICAL_VALIDITY_FAILED',
             decision='MATCHED_MEMORY_EXPERIMENT_COMPLETE_REQUIRES_INTERPRETATION',
        ...
        for key,value in [('RAMAS_STAGE6_2_STATUS',final['status']),('DECISION',final['decision']),
