...
import math
import numpy as np
from .legacy import legacy, EpisodicMemory, RegimeTrust, state_vector, validate_decision, blend_exposure
...
def payload_for(arm,period,i,config,base_config,risk):
    ...
    for event in arm.trust.events[before:]:
        g=event['regime']
        ids=[x['episode_id'] for x in completed if x['hard_regime']==g and x['action']!='ABSTAIN'][-config['trust']['lookback_episodes_per_regime']:]
        old=arm.previous_evidence.get(g,[])
        event['eligible_episode_ids']=ids
        event['fresh_evidence_since_prior_update']=bool(set(ids)-set(old))
        event['actual_latest_evidence_return_date']=max((x['return_date'] for x in completed if x['episode_id'] in ids),default=None)
        if not event['fresh_evidence_since_prior_update'] and event['old_beta']!=event['new_beta']:
            arm.stale_changes.append(dict(event))
        arm.previous_evidence[g]=ids
...
def run_pair(period,config,base_config,accounting,risk,journal,output):
    arms={name:Arm(name,RegimeTrust(config['trust'])) for name in config['arms']}
    rows=[]
    for i in range(len(period.frame)):
        ...
        if i==0 or (i+1)%25==0 or i+1==len(period.frame):
            print(f"STAGE62_PROGRESS={i+1}/{len(period.frame)} MEMORY_VALID={arms['memory'].valid} NO_MEMORY_VALID={arms['no_memory'].valid} NEW_CALLS={journal.new_calls} REUSED_CALLS={journal.reused_calls}",flush=True)
