...
import run_stage6_1 as run_stage6
...
class Stage6Tests(unittest.TestCase):
    ...
    def test_timeline_is_continuous_but_not_untouched_confirmation(self) -> None:
        cfg = config()
        self.assertTrue(cfg["frozen_claims"]["post2021_rows_are_reused_diagnostic"])
        self.assertFalse(cfg["frozen_claims"]["untouched_oos_confirmation"])
        self.assertEqual(
            cfg["continuous_timeline"]["year_end_action"],
            "REPORT_AND_CONTINUE_WITHOUT_RESET",
        )
        self.assertTrue(cfg["research_contract"]["no_yearly_economic_hard_stop"])

    def test_research_roles_are_not_mislabeled(self) -> None:
        cfg = config()["research_contract"]
        self.assertEqual(cfg["internal_control"], "pre_agent_ramoe")
        self.assertIn("buy_and_hold", cfg["external_baselines_deferred_until_pipeline_validation"])
        self.assertNotEqual(cfg["proposed_method"], cfg["internal_control"])

    def test_continuous_source_contract_covers_2021_through_2025(self) -> None:
        cfg = config()
        self.assertEqual(sum(item["rows"] for item in cfg["source_periods"]), 1608)
        self.assertEqual(cfg["continuous_timeline"]["first_decision_date"], "2021-01-01")
        self.assertEqual(cfg["continuous_timeline"]["last_return_date"], "2025-05-28")
        self.assertEqual(cfg["continuous_timeline"]["diagnostic_return_years"], [2022, 2023, 2024, 2025])

    def test_mixed_metrics_remain_a_finding(self) -> None:
        proposed = pd.Series(
            {
                "terminal_growth": 0.20,
                "sharpe_zero_cash_rate": 0.90,
                "maximum_drawdown_loss": 0.30,
                "daily_loss_cvar_95": 0.04,
            }
        )
        control = pd.Series(
            {
                "terminal_growth": 0.10,
                "sharpe_zero_cash_rate": 1.00,
                "maximum_drawdown_loss": 0.25,
                "daily_loss_cvar_95": 0.05,
            }
        )
        self.assertEqual(
            run_stage6.classify_metric_pattern(proposed, control),
            "MIXED_METRIC_FINDING_DO_NOT_REJECT_ARCHITECTURE",
        )
    ...
    def test_development_loop_reveals_completed_memory_only_on_next_decision(self) -> None:
        ...
        trace, calls, memory, _trust, _det_trust = run_stage6.run_continuous(
