...
SCRIPT_VERSION = "v3_corrected_controller_off_reconstruction"
...
def trace_records(matched, prefix):
    ...
    The trace's shadow fields are audited against the executed controller-ON
    fields.  They are an audit/reference copy of the executed path, not a
    controller-OFF path.  For controller OFF we therefore recompute the path
    from desired exposure and an independently propagated pretrade exposure.
    This avoids accidentally using the controller-ON pretrade exposure for the
    controller-OFF counterfactual.
    """
    records = {"controller_on": [], "controller_off": []}
    audit = {
        "prefix": prefix,
        "max_shadow_exposure_vs_executed_difference": 0.0,
        "max_shadow_return_vs_executed_difference": 0.0,
        "max_shadow_pretrade_vs_executed_difference": 0.0,
        "valid_fraction": None,
        "error_count": 0,
    }
    ...
    previous_direct_exposure = None

    for core, row in matched:
        asset_return = to_float(row, "asset_simple_return")
        desired = to_float(row, f"{prefix}_desired_exposure")
        executed_pretrade = to_float(row, f"{prefix}_pretrade_exposure")
        executed_exposure = to_float(row, f"{prefix}_exposure")
        executed_net_return = to_float(row, f"{prefix}_net_return")
        ...
        audit["max_shadow_exposure_vs_executed_difference"] = max(
            audit["max_shadow_exposure_vs_executed_difference"],
            abs(shadow_exposure - executed_exposure),
        )

        audit["max_shadow_pretrade_vs_executed_difference"] = max(
            audit["max_shadow_pretrade_vs_executed_difference"],
            abs(shadow_pretrade - executed_pretrade),
        )

        audit["max_shadow_return_vs_executed_difference"] = max(
            audit["max_shadow_return_vs_executed_difference"],
            abs(stored_shadow_return - executed_net_return),
        )

        # The counterfactual starts from the same initial holding, but after
        # the first decision it follows the direct desired-exposure path.
        if previous_direct_exposure is None:
            direct_pretrade = executed_pretrade
        else:
            direct_pretrade = previous_direct_exposure

        direct_turnover = abs(desired - direct_pretrade)
        direct_net_return = (
            (1.0 - COST_RATE * direct_turnover)
        ...
        records["controller_off"].append({
            ...
            "desired_exposure": desired,
            "net_return": direct_net_return,
            "exposure": desired,
            "turnover": direct_turnover,
        })
        ...
        previous_direct_exposure = desired * (1.0 + asset_return) / (
            1.0 + desired * asset_return
        )
